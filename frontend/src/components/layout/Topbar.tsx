    ...
    <header className="border-b border-line bg-surface">
      <div className="flex min-h-20 flex-wrap items-center justify-between gap-4 px-4 py-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <Button variant="secondary" loading={signingOut} onClick={() => void authStore.logout()}>
      ...
      {error && (
        <p role="alert" className="border-t border-line px-4 py-3 text-sm text-danger sm:px-6 lg:px-8">
          Sign-out failed. {error}
        </p>
      )}
      {notice && (
        <p role="status" className="border-t border-line px-4 py-3 text-sm text-warning sm:px-6 lg:px-8">
          {notice}
        </p>
      )}
      <details
      ...
      >
        <summary className="flex min-h-12 cursor-pointer list-none items-center gap-3 px-4 text-sm font-semibold focus-visible:outline-2 focus-visible:outline-offset-[-2px] focus-visible:outline-brand sm:px-6 [&::-webkit-details-marker]:hidden">
          ...
          Menu<span className="ml-auto text-xs font-normal text-muted">{title}</span>
        </summary>
